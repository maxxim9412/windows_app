...
set(IRIS_SDK_DOWNLOAD_URL "https://download.agora.io/sdk/release/iris_4.5.3-build.1_DCG_Windows_Video_Standalone_20260428_1050_31959.zip")
# iris dependencies end

# native dependencies start
set(NATIVE_SDK_DOWNLOAD_URL "https://download.agora.io/sdk/release/Agora_Native_SDK_for_Windows_rel.v4.5.3.70_32091_FULL_20260416_1933_1076734.zip")
...
function(download_and_extract URL TARGET_DIR EXTRACTED_DIR)
    ...
    # ПРАВКА: CMAKE_CURRENT_SOURCE_DIR здесь идёт через
    # flutter/ephemeral/.plugin_symlinks — симлинк, которым Flutter коротит
    # длинные пути пакетов на Windows. cmake -E tar (libarchive) отказывается
    # писать в путь, проходящий через симлинк ("Cannot extract through
    # symlink") — это защита от symlink-атак, а не баг конкретно тут.
    # Резолвим реальный путь перед распаковкой, чтобы её обойти.
    file(REAL_PATH "${EXTRACTED_DIR}" EXTRACTED_DIR_REAL)

    # Extract the file
    execute_process(
        COMMAND ${CMAKE_COMMAND} -E tar xzf "${TARGET_FILE}"
        WORKING_DIRECTORY ${EXTRACTED_DIR_REAL}
    )
endfunction()
